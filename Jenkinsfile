			...
			steps{
				catchError(buildResult: 'SUCCESS', stageResult: 'FAILURE') {
					sh '''
						...
						ls -lrt
						pwd
						docker run -e NO_COLOR=1 --mount type=bind,source="/var/lib/docker/volumes/jenkins_home_volume/_data/$wrkdir",target=/e2e/ -w /e2e/$wrkdir  --user "$(id -u):$(id -g)" custom_cypress
					'''
				}
			 }
		  post{
