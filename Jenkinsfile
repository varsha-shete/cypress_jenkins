			...
			steps{
				catchError(buildResult: 'SUCCESS', stageResult: 'FAILURE') {
					sh '''
						...
						docker run -e NO_COLOR=1 --mount jenkins_home_volume:/e2e -w /e2e  --user "$(id -u):$(id -g)" custom_cypress -C workspace/test_varsha/cypress_jenkins_test/cypress_jenkins/cypress.config.js -s workspace/test_varsha/cypress_jenkins_test/cypress_jenkins/*.cy.js
					'''
				}
			 }
			  post{
