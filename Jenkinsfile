			...
			steps{
				catchError(buildResult: 'SUCCESS', stageResult: 'FAILURE') {
					sh '''
						...
						ls -lrt
						pwd
						docker run -e NO_COLOR=1 --config CYPRESS_reportDir=e2etest/$wrkdir/reports -v jenkins_home_volume:/e2e/e2etest -w /e2e/ --user "$(id -u):$(id -g)" custom_cypress -C e2etest/$wrkdir/cypress.config.js --spec e2etest/$wrkdir/*.cy.js
					'''
				}
			 }
		         post{
